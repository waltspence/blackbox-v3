# Gaussian copula joint probability for binary legs
import math, random
...
def joint_win_prob(marginals, corr, mc_samples=10000, seed=42):
    n = len(marginals)
    R = [[float(corr[i][j]) for j in range(n)] for i in range(n)]
    for i in range(n):
        R[i][i]=1.0
        for j in range(i+1, n):
            if R[i][j] > 0.95: R[i][j]=0.95
            if R[i][j] < -0.95: R[i][j]=-0.95
            R[j][i]=R[i][j]
    L = _cholesky_psd(R)
    random.seed(seed)
    zthr = [_phi_inv(p) for p in marginals]
    wins = 0
    for _ in range(mc_samples):
        z0 = [random.gauss(0,1) for _ in range(n)]
        z = [sum(L[i][k]*z0[k] for k in range(i+1)) for i in range(n)]
        if all(zi <= zthr[i] for i,zi in enumerate(z)):
            wins += 1
    return wins/float(mc_samples)

...
def joint_samples(marginals, R, samples=10000, seed=13):
    n = len(marginals)
    for i in range(n):
        R[i][i]=1.0
        for j in range(i+1, n):
            R[i][j] = max(-0.95, min(0.95, R[i][j]))
            R[j][i] = R[i][j]
    L = _cholesky_psd(R)
    random.seed(seed)
    ...
    for _ in range(samples):
        ...
        # event occurs if Zi <= threshold
        outcomes.append([1 if z[i] <= zthr[i] else 0 for i in range(n)])
    return outcomes
